...
def readCamerasFromTransformsUnderwater(path, white_background, extension=".png"):
    ...
    for i, image_raw in enumerate(images_raw): 
        ...
        if num_true < 20:
           skip_index.append(i)
           continue
        bin_edges = np.linspace(0.75, 3, num=31)
        #hist_h = np.zeros((depth.shape[0] , len(bin_edges)-1))
        hist_h = im.sum(axis=1)
        hist_h, _ = np.histogram(hist_h, bins=bin_edges)
        hist_h = hist_h / hist_h.max()
        histograms.append([hist_h, None])
        images.append(im)
    ...
    for i in range(len(sensor_poses)):
        ...
        if add_sonar:
            cam_infos.append(CameraInfo(uid=i, R=R, T=T, FovY=hfov, FovX=vfov, image=image,
                            image_path=image_path, image_name=image_name, width=96, height=512, depth=depth,
                            is_sonar=True))
    ...
    PrincipalPoint= [305.3432, 249.1058]
    ImageSize = [512, 612]
    RadialDistortion= [0.0078, 0.3033]
